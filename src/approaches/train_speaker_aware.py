...
from src.dataset.audio2landmark.audio2landmark_dataset import Speaker_aware_branch_Dataset
from src.models.model_audio2landmark_speaker_aware import Audio2landmark_speaker_aware
...
class Speaker_aware_branch():
    ...
    def __init__(self, opt_parser):
        ...
        #     os.path.join(opt_parser.root_dir, 'puppets', '{}_face_close_mouth.txt'.format(opt_parser.puppet_name)))
        ...
        if(not opt_parser.test_end2end):
            ...
            self.train_data = Speaker_aware_branch_Dataset(dump_dir=self.dump_dir, dump_name=opt_parser.dump_file_name,
                                                    ...
                                                    status='train', use_11spk_only=opt_parser.use_11spk_only)
            ...
            self.eval_data = Speaker_aware_branch_Dataset(dump_dir=self.dump_dir, dump_name=opt_parser.dump_file_name,
                                                   num_window_frames=opt_parser.num_window_frames,
                                                   num_window_step=opt_parser.num_window_step,
                                                   status='val', use_11spk_only=opt_parser.use_11spk_only)
            ...
            self.eval_data = Speaker_aware_branch_Dataset(dump_dir='/content/MakeItTalk/examples/dump',
                                                    dump_name='random',
                                                    status='val',
                                                    num_window_frames=18,
                                                    num_window_step=1)
            # self.eval_data = Speaker_aware_branch_Dataset(dump_dir=r'/mnt/ntfs/Dataset/TalkingToon/VoxCeleb2/dump',
        ...
        if(not opt_parser.test_end2end):
            for i, batch in enumerate(self.eval_dataloader):
                global_id, video_name = self.eval_data[i][0][1][0], self.eval_data[i][0][1][1][:-4]
            ...
            self.emb_data = Speaker_aware_branch_Dataset(dump_dir='/content/MakeItTalk/examples/dump',
                                                         dump_name='celeb_normrot',
                                                         status='val',
                                                         num_window_frames=18,
                                                         num_window_step=1,
                                                         use_11spk_only=opt_parser.use_11spk_only)
    ...
    def __train_speaker_aware__(self, fls, aus, embs, face_id, reg_fls, rot_trans, rot_quats,
                                use_residual=False, is_training=True):

        fls_gt = fls[:, 0, :].detach().clone().requires_grad_(False)
        reg_fls_gt = reg_fls[:, 0, :].detach().clone().requires_grad_(False)
        ...
        d_real_dt, d_fake_dt, d_real_dl, d_fake_dl, g_fake_dt, g_fake_dl, d_ano_dt = 0., 0., 0., 0., 0., 0., 0.
        #
        # ''' ======================================================
        #                 Discriminator  D_T
        # ====================================================== '''
        #
        # if (self.opt_parser.train_DT > 0.0):
        #     for name, p in self.G.named_parameters():
        #         p.requires_grad = False
        #     for p in self.D_T.parameters():
        #         p.requires_grad = True
        #     for p in self.D_L.parameters():
        #         p.requires_grad = False
        #     self.opt_D_T.zero_grad()
        #
        #     # real
        #     fl_dis_pred, _, spk_encode = self.G(aus, embs * self.opt_parser.emb_coef, face_id, fls_without_traj, z, add_z_spk=True)
        #
        #     d_real = self.D_T(fls_without_traj, spk_encode.detach())
        #     d_real_dt = self.loss_mse(d_real, torch.ones_like(d_real).to(device)) * 2.0 * self.opt_parser.train_DT
        #     if (is_training):
        #         d_real_dt.backward()
        #
        #     # fake
        #     d_fake = self.D_T(fl_dis_pred.detach() + face_id, spk_encode.detach())
        #     d_fake_dt = self.loss_mse(d_fake, torch.zeros_like(d_fake).to(device)) * self.opt_parser.train_DT
        #     if (is_training):
        #         d_fake_dt.backward()
        #
        #     # # another embedding
        #     # another_emb = np.tile(another_emb, (embs.shape[0], 1))
        #     # another_emb = torch.tensor(another_emb, dtype=torch.float, requires_grad=False).to(device)
        #     # d_ano = self.D_T(another_emb * self.opt_parser.emb_coef, fls_without_traj)
        #     # d_ano_dt = self.loss_mse(d_ano, torch.zeros_like(d_ano).to(device))
        #     # if (is_training):
        #     #     d_ano_dt.backward()
        #
        #     self.opt_D_T.step()
        ...
        #                 Discriminator  D_L
        # ====================================================== '''
        # if (self.opt_parser.train_DL>0.0):
        #     for p in self.D_L.parameters():
        #         p.requires_grad = True
        #     for p in self.D_T.parameters():
        #         p.requires_grad = False
        #     self.opt_D_L.zero_grad()
        #
        #     # real
        #     fl_dis_pred, _, spk_encode = self.G(aus, embs * self.opt_parser.emb_coef, face_id, fls_without_traj, z, add_z_spk=True)
        #
        #     d_real = self.D_L(fls_without_traj)
        #     d_real_dl = self.loss_mse(d_real, torch.ones_like(d_real).to(device)) * 1.0 * self.opt_parser.train_DL
        #     if (is_training):
        #         d_real_dl.backward()
        #
        #     # fake
        #     d_fake = self.D_L(fl_dis_pred + face_id)
        #     d_fake_dl = self.loss_mse(d_fake, torch.zeros_like(d_fake).to(device)) * self.opt_parser.train_DL
        #     if (is_training):
        #         d_fake_dl.backward()
        #
        #     self.opt_D_L.step()
        ...
        if (use_residual):
            ...
            ''' CALIBRATION in TEST TIME '''
            if (not is_training):

                smooth_length = int(min(fl_dis_pred.shape[0] - 1, 51) // 2 * 2 + 1)
                fl_dis_pred = savgol_filter(fl_dis_pred.cpu().numpy(), smooth_length, 3, axis=0)
                fl_dis_pred *= self.opt_parser.amp_pos

                # close pose-branch mouth
                fl_dis_pred = fl_dis_pred.reshape((-1, 68, 3))
                index1 = list(range(60-1, 55-1, -1))
                index2 = list(range(68-1, 65-1, -1))
                mean_out = 0.5 * (fl_dis_pred[:, 49:54] + fl_dis_pred[:, index1])
                mean_in = 0.5 * (fl_dis_pred[:, 61:64] + fl_dis_pred[:, index2])
                fl_dis_pred[:, 49:54] = fl_dis_pred[:, index1] = mean_out
                fl_dis_pred[:, 61:64] = fl_dis_pred[:, index2] = mean_in
                ...
                baseline_pred_fls, _ = self.C(aus[:, 0:18, :], content_branch_face_id)
                # option 2
                # baseline_pred_fls -= mean_face_id
        ...
        if (self.opt_parser.pos_dim == 7):
            pos_pred[:, 0:4] = torch.nn.functional.normalize(pos_pred[:, 0:4], p=2, dim=1)
        return fl_dis_pred, pos_pred, face_id[0:1, :], (loss, loss_reg_fls, loss_laplacian, loss_pos)
